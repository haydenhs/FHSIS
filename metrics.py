...
def compare_psnr(x_true, x_pred, data_range):
    """
    :param x_true: Input image must have three dimension (H, W, C)
    :param x_pred:
    :return:
    """
    x_true, x_pred = x_true.astype(np.float32), x_pred.astype(np.float32)
    return peak_signal_noise_ratio(image_true=x_true, image_test=x_pred, data_range=data_range)
...
def compare_mssim(x_true, x_pred, data_range, multidimension):
    ...
    :return:
    """
    mssim = [structural_similarity(im1=x_true[:, :, i], im2=x_pred[:, :, i], data_range=data_range, multidimension=multidimension)
            for i in range(x_true.shape[2])]

    return np.mean(mssim)


def compare_ssim(x_true, x_pred, data_range, multidimension):
    """
    :param x_true:
    :param x_pred:
    :param data_range:
    ...
    :return:
    """
    return structural_similarity(im1=x_true, im2=x_pred, data_range=data_range, multidimension=multidimension)
...
def quality_assessment(x_true, x_pred, data_range, ratio, multi_dimension=False, block_size=8):
    ...
    :return:
    """
    result = {'PSNR': compare_psnr(x_true=x_true, x_pred=x_pred, data_range=data_range),
              'SSIM': compare_ssim(x_true=x_true, x_pred=x_pred, data_range=data_range,
