...
class HSIDataset(Dataset):
    def __init__(self, input_dir, sr_factor=32, dataset_name='CAVE', lr_type='bicubic', rgb_type='Nikon', use_aug=True, is_eval=False):
        if dataset_name not in ['CAVE', 'Harvard']:
            raise NotImplementedError
        with h5py.File(input_dir, 'r') as data:
            hsi = data['hsi'][...]
            train_total = int(hsi.shape[0] * 0.9)
            if is_eval:
                hsi = np.float32(hsi[train_total:, :, :, :])
            else:
                hsi = np.float32(hsi[:train_total, :, :, :])
...
if __name__ == "__main__":
    dataset = HSIDataset(input_dir='/home/CAVE_h5/CAVE_train.h5', sr_factor=32, dataset_name='CAVE', lr_type='wald', rgb_type='Nikon', is_eval=True)
