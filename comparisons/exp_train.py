...
def train(args):
    ...
    model = GPPNN()
    model_type =  'GPPNN_' + args.dataset + '_' + args.lr_type + '_' + args.rgb_type + '_x' + str(args.sr_factor)

    if torch.cuda.device_count() > 1:
        print("===> Using {} GPUs: {}".formta(torch.cuda.device_count(), args.gpus))
    ...
    for epoch in range(starting_epoch, args.num_epoch):
        ...
        #scheduler.step()
        val_loss = validate(args, test_loader, model)
...
if __name__ == '__main__':
    ...
    # Set logger
    log_name = "ExpGPPNN_train_%s.txt" % time.ctime()
